...
namespace roi_depth_query
{
    ...
    public:
            ...
            : Node("roi_depth_node", opts)
        {
            ...
            // In constructor, instead of extr_sub_:
            extr_param_timer_ = create_wall_timer(
                ...
                [this]()
                {
                    ...
                    if (rot.size() == 9 && rot[0] != 0.0)
                    {
                        std::lock_guard lk(lut_mutex_);
                        rs2_extrinsics d2c{};
                        for (int i = 0; i < 9; ++i)
                            d2c.rotation[i] = float(rot[i]);
                        for (int i = 0; i < 3; ++i)
                            d2c.translation[i] = float(trans[i]);
                        depth_to_color_ = d2c;
                        rs2_extrinsics c2d{};
                        for (int r = 0; r < 3; ++r)
                            for (int c = 0; c < 3; ++c)
            ...
            // Declare with identity defaults so the timer doesn't crash before they arrive
    ...
    private:
            ...
            const realsense2_camera_msgs::msg::Extrinsics &msg)
        {
            rs2_extrinsics ex{};
            for (int i = 0; i < 9; ++i)
                ex.rotation[i] = msg.rotation[i];
            for (int i = 0; i < 3; ++i)
                ex.translation[i] = msg.translation[i];
            return ex;
        }
        ...
        void tryBuildLut()
        {
            if (!depth_intr_ready_ || !color_intr_ready_)
                return;
